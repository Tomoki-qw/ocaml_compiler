module Lexer = struct
  ...
  let lookahead () = try let c = read () in unread c; c with End_of_file -> '$'
    ...
    let c = lookahead () in
      ...
      (*抽象構文木の印字関数*)
      module P = Printf
      let rec print_ast_list lst1 lst2 = match (lst1,lst2) with ([],[]) -> ()
      |(x1::rest1,x2::rest2) -> if x1 = x2 then (print_ast_list rest1 rest2)
      else (match (x1,x2) with 
      |((Atom t),(Var s)) -> (P.printf "%s" s;print_string
      "=";P.printf "%s" t;P.printf "\n";print_ast_list rest1 rest2)
      |(_,_) -> ())
      |(_,_) -> ()
      ...
      let rec out_solve (program, question, result, depth) = let aaa = result in
