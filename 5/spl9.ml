    ...
    let c = lookahead () in
      ...
      let rec print_ast_list lst1 lst2 = 
        match (lst1,lst2) with ([],[]) -> 
                               (print_string "continue?" ; flush stdout;
                               let a = ref ' ' in
                                while !a != ';' && !a != '.' do
                                a := input_char stdin
                                done;
      ...
      let rec run () =
        Lexer._ISTREAM := open_in ("isono.ml");Parser.advance () ; Parser.command ()  ;print_string "load ";print_string "isono.pl" ; print_string "." ;print_string "\n?- ";
        while true do
        flush stdout; Lexer._ISTREAM := stdin;
        Parser.advance () ; Parser.command ()  ; print_string "complete." ; print_string "\n?- "
        done
      let _ = run()
      ;;