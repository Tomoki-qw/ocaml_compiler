module Lexer = struct
  ...
  let lookahead () = try let c = read () in unread c; c with End_of_file -> '$'
    ...
    let c = lookahead () in
      ...
      print_ast hd; List.iter (fun x -> 
                               (print_string ","; print_ast x)) tl; print_string ").")
      | (App(s, [])) -> (P.printf "%s" s ;print_string "( ).")
      | (Atom s) -> P.printf "%s" s
      | (Var s) -> P.printf "%s" s
      ...
      let mgu (a,b) =
         let rec ut (one, another, unifier) = match (one, another) with
          ([], []) -> (true, unifier)
      | (term::t1, Var(name)::t2) -> let r = fun x -> 
                                      sub name term (unifier x) 
                                      in ut(List.map r t1,List.map r t2, r)
      | (Var(name)::t1, term::t2) -> let r = fun x -> 
                                      sub name term (unifier x) 
                                      in ut(List.map r t1,List.map r t2, r) 
      | (Atom(n)::t1, Atom(m)::t2) ->
